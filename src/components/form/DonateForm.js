    ...
    render() {
        ...
        return ( <
            ...
            div className = 'form-background' > {
                this.state.loading === true ? < div className = 'form-header' > The nodes are now noding < /div> : <div className='form-header'>Investor filling form <br/ > Fill with the following < /div> }
                ...
                <PeriodLoader/>
                </div> <div className = { this.state.loading === true ? 'hide' : 'flex-forms' } >
                <div className = 'form-align' >
                </div> { / * < hr className = 'horizontal-line' / > * / } 
		<div className = 'form-caption' > Name < /div> <input
		...
		<div className = 'form-caption' > Message < /div> <
                input type = { this.state.errorMessage === "Empty fields" ? 'text' : '' }
                maxLength = "180"
                value = { this.state.description }
                onChange = { event => this.setState({ description: event.target.value }) } >
                </input> 
		<div className = 'form-caption' > Amount in Harmony < /div> <
                input type = { this.state.errorMessage ? 'text' : '' }
                value = { this.state.value }
                onChange = { event => this.setState({ value: event.target.value }) } >
                </input>  
                <div className = 'form-caption' > Harmony Address < /div> <
                input type = { this.state.errorMessage ? 'text' : '' }
                maxLength = "20"
                ...
                onClick = { this.onDonateMessageSend } > Donate < /button> </div > </div> </div>
            )
        }
    }